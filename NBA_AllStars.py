...
def scrape_api_create_database(db_filename, cur, conn):
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS Teams (team_id INTEGER PRIMARY KEY, c_win INTEGER, c_loss INTEGER, league_W_perc FLOAT, league_L_perc FLOAT, season INTEGER)")
    cur.execute("SELECT COUNT(*) FROM Teams")
    row_count = cur.fetchone()
    if row_count[0] < 120:
        for i in range(20):
            cur.execute("INSERT INTO Teams (team_id, c_win, c_loss, league_W_perc, league_L_perc, season) VALUES (?,?,?,?,?,?)",(row_count[0] + i, c_wins[i], c_losses[i], league_W_percs[i], league_L_percs[i], season[i]))
        conn.commit()
...
def create_database(cur, conn):
    ...
    for i in range(len(dic.keys())):
        cur.execute("INSERT OR IGNORE INTO allStars (team_id, name, num_allstarplayers) VALUES (?,?,?)",(i, a[i], b[i]))
    conn.commit()
...
def calcuate_WLRatio(cur,conn):
    tups = []
    cur.execute("SELECT allStars.name, allStars.num_allstarplayers, Teams.c_win, Teams.c_loss, Teams.season FROM allStars INNER JOIN Teams ON Teams.team_id=allStars.team_id")
    info = cur.fetchall()
    new = []
...
def findLeagueInfo(conn,cur):
    tups = []
    cur.execute("SELECT allStars.name, allStars.num_allstarplayers, Teams.league_W_perc, Teams.league_L_perc FROM Teams INNER JOIN allStars ON Teams.team_id=allStars.team_id")
    info = cur.fetchall()
    return info
...
def boxplot_w(ratio):
    ...
    plt.barh(allstars, wins, color = "#cf2913")
    plt.show()
    plt.clf()  
...
def boxplot_l(ratio):
    ...
    plt.barh(allstars, losses, color = "#cf2913")
    plt.show()
    plt.clf()
